...
theme.config = {
  ...
  mediaQuerySmall: 'screen and (max-width: '+ 768 +'px)',
  youTubeReady: false,
  vimeoReady: false,
  vimeoLoading: false,
...
(function(){
  ...
  theme.sizeDrawer = function() {
    ...
    setTimeout(function() {
      var headerDrawer = document.querySelector('#MobileNav .site-header__drawer-animate').offsetHeight;
      document.documentElement.style.setProperty('--headerDrawer', `${headerDrawer}px`);
      if (window.innerHeight < (headerDrawer + 110)) {
        document.querySelector('.site-header__mobile-nav').classList.add('max-height');
      } else {
        document.querySelector('.site-header__mobile-nav').classList.remove('max-height');
      }
    }, 250);
    document.documentElement.style.setProperty('--headerHeight', `${header}px`);
    document.documentElement.style.setProperty('--maxDrawerHeight', `${window.innerHeight}px`);
