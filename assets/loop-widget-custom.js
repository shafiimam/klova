    ...
    if (!isOneTimeOrder) {
        return (
            ...
            window?.loopPropsUI?.translationData
                ?.widget_add_to_cart_button_for_subscription || "Add subscription to Carts"  
        );
...
function updatePriceInUI({ productId }) {
  ...
  var sellingPlannn = window.loopProps[productId].sellingPlan;
  let quantityDiv= document.querySelector('input[name="quantity"]')
  let quanTity = parseInt(quantityDiv.value)
  let quantityInputs = document.querySelectorAll('.main-product__box .js-qty__adjust');
  quantityInputs.forEach((input) => {
    input.addEventListener('click', () => {
        let quantityDiv = input.closest('.main-product__box').querySelector('input[name="quantity"]');
        let quanTity = parseInt(quantityDiv.value);
        
  if (Object.keys(sellingPlannn).length !== 0) {
    // The sellingPlan objec]t is not empty, display the price
     const parentElement =document.querySelector(`[data-loop-product-id="${productId}"]`) ||document;
     const addToCartButton = getAddToCartButton(parentElement);
    console.log(sellingPlannn.price)
     updateButtonInnerHTML(addToCartButton, loopFormatMoney(sellingPlannn.price*quanTity,true));

}
  else {
    const parentElement =document.querySelector(`[data-loop-product-id="${productId}"]`) ||document;
    const addToCartButton = getAddToCartButton(parentElement);
    updateButtonInnerHTML(addToCartButton, loopFormatMoney(variant.price*quanTity, true));
    // The sellingPlan object is empty, do nothing
